...
export async function analyzeModularity(projectPath: string): Promise<ModularityResult> {
  ...
  if (uiImportingLogicDirectly > 0) {
    issues.push(
      `${uiImportingLogicDirectly} arquivo(s) de UI importam serviços/store diretamente (>3 imports). Use hooks como intermediários.`
    );
  }

  if (hooksImportingUi > 0) {
    issues.push(
      `${hooksImportingUi} hook(s) importam componentes de UI — inversão de dependência. Hooks não devem depender de UI.`
    );
  }
